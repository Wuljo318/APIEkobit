...
namespace DataEkobit.Repositories
{
    public class UserBase : AppDbBase<User>, IUserBase
    {
        ...
        {
            return await _appDbContext.Set<User>().Include(u => u.Country).AsNoTracking().ToListAsync();
        }

        public override async Task<User> FindById(Expression<Func<User, bool>> expression)
        {
            return await _appDbContext.Set<User>().Where(expression).Include(u => u.Country).AsNoTracking().FirstOrDefaultAsync();
        }
    }
}
